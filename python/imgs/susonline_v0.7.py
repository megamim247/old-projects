...
#weapon database:
isshot=[0,1,0,0,1]
firerate=[5,11,19,1,5]
damage=[3,1,15,1,0.11111111111111111111]
...
def handle_bullets():
    ...
    if wid==1:
        dp=0.25
    if wid==4:
        dp=(0.074074074074074074074074+0.01234567890123456789)*damage[wid]
    #guy
    ...
    for bullet in otherguybullets:
        ...
        if guy.colliderect(bullet):
            ...
            otherguyhits+=damage[otherguywid]
        elif bullet.x > WIDTH or bullet.x<0:
            otherguybullets.remove(bullet)
            del otherguybulletdirection[b_num]
        for n in range(len(borderyx)):
...
send(msg)
#otherguyweapon
otherweapon=client.recv(HEADER).decode(FORMAT)
if otherweapon[0]=="a":
    otherguywid=0
    
if otherweapon[0:2]=="sh":
    otherguywid=1
    
if otherweapon[0:2]=="sn":
    otherguywid=2

if otherweapon[0:2]=="sm":
    otherguywid=3

if otherweapon[0:2]=="bs":
    otherguywid=4

print(otherweapon[0:2])
print(otherguywid)
#receive
def receive():
    global otherguyx
    ...
    while True:
        ...
        if len(msg)>3:
            if int(msg)<2000:
                ...
                msg=msg[3:]
                if msg[0]=="1":
                    if isshot[otherguywid]==1:
                        bullet1=pygame.Rect(otherguy.x,(otherguy.y+otherguy.height//2)+10,15,4)
                        bullet2=pygame.Rect(otherguy.x,(otherguy.y+otherguy.height//2),15,4)
                        bullet3=pygame.Rect(otherguy.x,(otherguy.y+otherguy.height//2)-10,15,4)
                        otherguybullets.append(bullet1)
                        otherguybulletdirection.append(otherguyleft)
                        otherguybullets.append(bullet2)
                        otherguybulletdirection.append(otherguyleft)
                        otherguybullets.append(bullet3)
                        otherguybulletdirection.append(otherguyleft)
                    else:
...
while run:
    ...
    if pressed[pygame.K_w] and (guy.y+29>HEIGHT or collideb):
        ...
        jump=True
        collideb=False
    if pressed[pygame.K_SPACE] and (runs%firerate[wid]==0 or runs<2):
        bullet=pygame.Rect(guy.x,(guy.y+guy.height//2),15,4)
        bullets.append(bullet)
        bulletdirection.append(left)
        if isshot[wid]==1:
            bullet1=pygame.Rect(guy.x,(guy.y+guy.height//2)+10,15,4)
            bullet2=pygame.Rect(guy.x,(guy.y+guy.height//2),15,4)
            bullet3=pygame.Rect(guy.x,(guy.y+guy.height//2)-10,15,4)
    ...
    if pressed[pygame.K_SPACE]:
        pass
    elif runs%firerate[wid]==0 or runs<2:
        runs=0
