...
import turtle
deck=uno_util.deck
hand1=uno_util.hand1
hand2=uno_util.hand2
plays=[]
...
while len(hand1)>0 and len(hand2)>0 and deckcount>0:
    while (len(hand1)>0 and len(hand2)>0) and len(deck)>0:
        ...
        if len(hand2)>0:
            print(hand1)
            ((play,hand1,deck,skip))=uno_util.play1(hand1,hand2,play,deck)
            if skip ==False:
                plays.append(play)
                print(play)
            else:
                play=plays[-1].rsplit(":")[0]+":s"
                plays.append(play)
                print("skip")
            hand1len.append(len(hand1))
        # player2
        if len(hand1)>0:
            print(hand2)
            ((play,hand2,deck,skip))=uno_util.play2(hand2,play,deck)
            if skip ==False:
                plays.append(play)
                print(play)
            else:
            ...
            hand2len.append(len(hand2))
    deckcount-=1
    deck=uno_util.createnewdeck(hand1,hand2)
print(plays)
print(hand1len)
print(hand2len)
